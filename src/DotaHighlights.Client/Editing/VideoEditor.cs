...
public sealed class VideoEditor
{
    ...
    public VideoEditor(string ffmpegPath, ILogger log)
    {
        ...
        _log = log;
        // ffprobe vive junto a ffmpeg.
        var dir = Path.GetDirectoryName(ffmpegPath);
        _ffprobePath = string.IsNullOrEmpty(dir) ? "ffprobe" : Path.Combine(dir, "ffprobe.exe");
    }

    /// <summary>
    /// Crea las 3 ediciones derivadas del clip. Devuelve las que se generaron bien.
    /// </summary>
    ...
    public async Task<IReadOnlyList<EditedClip>> EditAllAsync(
        string source, double moneyShotSeconds, CancellationToken ct = default)
    {
        double dur = await ProbeDurationAsync(source, ct);
        if (dur <= 1) dur = 6; // fallback defensivo

        // Ventana del momento clave, saneada a los límites del clip.
        ...
        var jobs = new (string id, string name, string outFile, string[] args)[]
        {
            ("slowmo", "🐢 Slow-mo", $"{baseName}_slowmo.mp4",
                FilterComplexArgs(source, Path.Combine(outDir, $"{baseName}_slowmo.mp4"),
                    $"[0:v]trim=0:{F(s0)},setpts=PTS-STARTPTS[a];" +
                    $"[0:v]trim={F(s0)}:{F(s1)},setpts=(PTS-STARTPTS)/0.35,eq=saturation=1.2:contrast=1.05[b];" +
                    $"[0:v]trim={F(s1)},setpts=PTS-STARTPTS[c];" +
                    $"[a][b][c]concat=n=3:v=1[v]")),

            ("zoom", "🔍 Zoom épico", $"{baseName}_zoom.mp4",
                FilterComplexArgs(source, Path.Combine(outDir, $"{baseName}_zoom.mp4"),
                    $"[0:v]trim=0:{F(s0)},setpts=PTS-STARTPTS,eq=saturation=1.25:contrast=1.08[a];" +
                    $"[0:v]trim={F(s0)}:{F(z1)},setpts=PTS-STARTPTS,scale=w=iw*1.3:h=ih*1.3,crop=iw/1.3:ih/1.3,eq=saturation=1.4:contrast=1.12[b];" +
                    $"[0:v]trim={F(z1)},setpts=PTS-STARTPTS,eq=saturation=1.25:contrast=1.08[c];" +
                    $"[a][b][c]concat=n=3:v=1[v]")),

            ("cinematic", "🎞️ Cinematic", $"{baseName}_cinematic.mp4",
                SimpleFilterArgs(source, Path.Combine(outDir, $"{baseName}_cinematic.mp4"),
                    ...
                    $"fade=t=in:st=0:d=0.6,fade=t=out:st={F(fadeOut)}:d=0.6")),
        };
        ...
        foreach (var job in jobs)
        {
            ...
            _log.Information("Editando ({Style}) -> {Out}", job.id, outPath);
            int exit = await RunAsync(_ffmpegPath, job.args, ct);
    ...
    private static string[] FilterComplexArgs(string src, string dst, string filter) =>
        new[]
        {
    ...
    private static string[] SimpleFilterArgs(string src, string dst, string vf) =>
        new[]
        {
            "-y", "-hide_banner", "-loglevel", "error",
            "-i", src,
